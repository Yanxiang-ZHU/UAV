...
class TrainingThread(QtCore.QThread):
    ...
    def run(self):
        ...
        # create checkpoint callback to periodically save model during training
        save_freq = self.cfg.getint('options', 'save_freq', fallback=500)
        checkpoint_callback = CheckpointCallback(save_freq, save_path=model_path, name_prefix='model_checkpoint')
        ...
        if self.cfg.getboolean('options', 'use_wandb'):
            ...
            wb_callback = WandbCallback(
                ...
                gradient_save_freq=max(1, save_freq//2),
            ...
            model.learn(
                total_timesteps,
                log_interval=1,
                callback=CallbackList([wb_callback, checkpoint_callback])
            )
        else:
